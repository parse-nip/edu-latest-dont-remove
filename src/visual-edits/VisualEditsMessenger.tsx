...
import { FilePenLine, FilePlus2, FileMinus2, ChevronDown, ChevronUp } from "lucide-react";
...
export default function HoverReceiver() {
  // Enhanced Changed Files panel state
  const [changedFiles, setChangedFiles] = useState<Array<{ path: string; status?: "modified" | "created" | "deleted" | string }>>([]);
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      // Listen for changed files payloads from parent
      if (e.data?.type === "ORCHIDS_CHANGED_FILES") {
        const files = Array.isArray(e.data.files) ? e.data.files : [];
        // Normalize: expect array of { path, status }
        const normalized = files
          .map((f: any) => ({ path: String(f.path || f.file || ""), status: (f.status || "modified") as string }))
          .filter((f: any) => f.path);
  ...
  return (
    <>
      ...
      {/* Changed Files Panel UI */}
      {showChanged && changedFiles.length > 0 && (
        <div className="fixed bottom-4 right-4 z-[100010] w-[22rem] max-w-[calc(100vw-2rem)] rounded-xl border border-border bg-card text-card-foreground shadow-xl">
          <div className="flex items-center justify-between px-4 py-2 rounded-t-xl bg-gradient-to-r from-muted/60 to-transparent border-b border-border">
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold">Changed Files</span>
              <span className="inline-flex items-center justify-center text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-primary/10 text-primary">
                {changedFiles.length}
              </span>
            </div>
            <button
              onClick={() => setShowChanged(false)}
              className="inline-flex h-6 w-6 items-center justify-center rounded-md hover:bg-muted text-muted-foreground"
              ...
              <li key={index} className="px-4 py-2 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  {file.status === "modified" && (
                    <FilePenLine className="mt-0.5 w-4 h-4 text-blue-500" />
                  )}
                  {file.status === "created" && (
                    <FilePlus2 className="mt-0.5 w-4 h-4 text-green-500" />
                  )}
                  {file.status === "deleted" && (
                    <FileMinus2 className="mt-0.5 w-4 h-4 text-red-500" />
                  )}
                  {/* Path + status badge */}
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <code className="font-mono text-xs text-foreground truncate" title={file.path}>
                        {file.path}
                      </code>
                      <span
                        className={
                          "inline-flex whitespace-nowrap px-1.5 py-0.5 rounded text-[10px]" +
                          (file.status === "created"
      ...
      {/* Collapsed reopen pill */}
      {!showChanged && changedFiles.length > 0 && (
        <button
          onClick={() => setShowChanged(true)}
          className="fixed bottom-4 right-4 z-[100010] inline-flex items-center gap-1 rounded-full border border-border bg-card/90 backdrop-blur px-3 py-1.5 text-xs shadow-lg hover:bg-card"
          aria-label="Expand changed files"
        >
          <ChevronUp className="w-4 h-4 text-muted-foreground" />
          <span className="font-medium">Changed Files</span>
          <span className="ml-1 inline-flex items-center justify-center text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-primary/10 text-primary">
            {changedFiles.length}
          </span>
        </button>
      )}
